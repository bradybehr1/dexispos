export class RegisterModule {
    ...
    getTotals() {
        ...
        const tax = subtotal * (parseFloat(this.router.globalSettings.taxRate) / 100);
                        ...
                        <div class="grid grid-cols-2 xl:grid-cols-3 gap-2">
                            ${filtered.map(p => `
                                <div onclick="window.registerModuleRef.addToCart('${p.id}')" class="bg-[#141619] border border-[#2a2e35] p-3 rounded flex flex-col justify-between hover:border-slate-500 transition cursor-pointer group min-h-[95px]">
                                    ...
                                    <div class="flex items-end justify-between mt-2 font-mono"><span class="text-xs font-black text-blue-400">${this.router.globalSettings.currencySymbol}${p.price.toFixed(2)}</span><span class="text-[10px] text-slate-500">QTY: ${p.stock}</span></div>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                ...
                <div class="w-full lg:w-[360px] bg-[#141619] flex flex-col justify-between overflow-hidden flex-shrink-0 border-t lg:border-t-0 border-[#2a2e35]">
                    ...
                    <div class="flex-1 overflow-y-auto p-3 space-y-1">
                        ${this.cart.map(item => `
                            <div class="bg-[#1c1f24] border border-[#2a2e35] p-2.5 rounded font-mono text-[11px] flex justify-between items-center">
                                <div class="max-w-[180px]"><h5 class="font-bold text-slate-300 uppercase truncate">${item.name}</h5><span class="text-[10px] text-slate-500">${this.router.globalSettings.currencySymbol}${item.price.toFixed(2)} x ${item.quantity}</span></div>
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="font-bold text-slate-200 w-16 text-right">${this.router.globalSettings.currencySymbol}${((item.price * item.quantity) - item.discount).toFixed(2)}</span>
                                </div>
                            </div>
                        `).join('')}
                    </div>
                    <div class="p-3 bg-[#0c0d0e] border-t border-[#2a2e35] space-y-3">
                        <div class="space-y-1 font-mono text-[11px] text-slate-400">
                            <div class="flex justify-between"><span>SUBTOTAL FLUSH:</span><span class="text-slate-300">${this.router.globalSettings.currencySymbol}${subtotal.toFixed(2)}</span></div>
                            <div class="flex justify-between border-t border-[#2a2e35] pt-2 text-xs font-black text-slate-200"><span>BAL DUE VALUE:</span><span class="text-blue-400 font-bold">${this.router.globalSettings.currencySymbol}${total.toFixed(2)}</span></div>
                        </div>
                        <button onclick="window.registerModuleRef.checkoutModalOpen=true; window.state.render()" ${this.cart.length === 0 ? 'disabled' : ''} class="w-full py-3 bg-blue-600 disabled:bg-[#1c1f24] disabled:border-[#2a2e35] disabled:text-slate-600 border border-blue-500 text-white rounded font-mono font-bold text-xs uppercase tracking-widest jmd-action-active transition">F1: Tender Settlement</button>
                    </div>
                </div>
            </div>
            ${this.checkoutModalOpen ? this.templateCheckoutModal(total) : ''}
            ${this.receiptModalOpen ? this.templateReceiptModal() : ''}
        `;
    }

    templateCheckoutModal(total) {
        return `
            <div class="fixed inset-0 bg-[#0c0d0e]/80 backdrop-blur-xs z-50 flex items-center justify-center p-4">
                <div class="bg-[#141619] border border-[#2a2e35] p-5 rounded w-full max-w-sm text-center space-y-4 shadow-2xl">
                    ...
                    <div class="bg-[#0c0d0e] border border-[#2a2e35] p-3 font-mono text-xl text-blue-400 font-bold">${this.router.globalSettings.currencySymbol}${total.toFixed(2)}</div>
                    <div class="grid grid-cols-1 gap-2">
                        <button onclick="window.registerModuleRef.processCheckout('Card')" class="py-3 bg-[#1c1f24] hover:bg-[#2a2e35] text-slate-200 border border-[#2a2e35] rounded text-xs font-mono font-bold uppercase tracking-wider transition">Electronic Integrated EFT</button>
                        <button onclick="window.registerModuleRef.processCheckout('Cash')" class="py-3 bg-[#1c1f24] hover:bg-[#2a2e35] text-slate-200 border border-[#2a2e35] rounded text-xs font-mono font-bold uppercase tracking-wider transition">Cash Ledger Settlement</button>
